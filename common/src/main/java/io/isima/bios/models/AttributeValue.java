...
public interface AttributeValue {
  /**
   * Returns the value as a boolean. If the native type is not boolean, an attempt will be made to
   * convert it to a boolean.
   *
   * @return value as boolean
   * @throws InvalidDataRetrievalException if conversion is not possible and the native type is not
   *     boolean
   */
  boolean asBoolean() throws InvalidDataRetrievalException;

  /**
   * Returns the value as a long. If the native type is not long, an attempt will be made to convert
   * it to boolean.
   *
