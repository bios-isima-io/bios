...
class TestWebhookChainedProcess(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.TENANT_NAME = "test"
        cls.SIGNAL_NAME = "simpleSignal"
        cls.conn = HttpConnection(get_webhook_url())
        cls.WEBHOOK_PATH = "simple-signal/process-chain"
    ...
    def test_positive_filter(self):
        ...
        with bios.login(ep_url(), f"{admin_user}@{self.TENANT_NAME}", admin_pass) as client:
            ...
            statement = (
                ...
                .where(f"testName = 'step1: Original String'")
            ...
            self.assertEqual(records[0].get("testParam"), "step2(step1: Original String)")
