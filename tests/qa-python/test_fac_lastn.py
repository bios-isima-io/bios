...
class TestFeatureAsContextLastN(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with bios.login(ep_url(), cls.ADMIN_USER, admin_pass) as admin:
            ...
            initial_events = [
                "regionA,mobile,jack,10,9991234567",  # ...0
                "regionA,laptop,paul,20,9991234566",  # ...1
                "regionA,smarttv,nancyA,30,9991234565",  # 2
                "regionA,battery,paul,15,9991234569",  # ..3
                "regionA,tablet,paul,40,9991234566",  # ...4
                "regionA,tv,nancyB,50,9991234565",  # .....5
                "regionB,tablet,nick,60,9991234564",  # ...6
                "regionB,tv,anne,15,9991234569",  # .......7
                "regionB,battery,paul,15,9991234569",  # ..8
                "regionB,tv,eddy,45,9991234563",  # .......9
                "regionB,mobile,paul,80,9991234566",  # ..10
                "regionB,battery,anne,15,9991234569",  # .11
                "regionA,smarttv,gary,90,9991234562",  # .12
            ]
            responses = [
            ...
            second_events = [
                "regionA,smarttv,michael,3,8881234565",  # ...0
                "regionB,smarttv,oliver,2,8881234566",  # ....1
                "regionA,laptop,nancyC,1,9991234565",  # .....2
                "regionB,smartphone,anne,5,9991234569",  # ...3
                "regionA,mobile,eddy,8,9991234563",  # .......4
                "regionB,battery,michaelJr,2,8881234565",  # .5
                "regionA,mobile,oliver,3,8881234566",  # .....6
                "regionB,battery,june,3,8881234565",  # ......7
                "regionA,smarttv,paul,4,9991234566",  # ......8
                "regionB,smartphone,jack,7,9991234567",  # ...9
            ]
    ...
    def test_last_four_phone_numbers_on_product_id(self):
        """Test LastN data sketches with string keys"""
        ...
        try:
            ...
            self.assertEqual(values0[2].get("t"), self.timestamps_second[8])
            self.assertEqual(values0[2].get("v"), 9991234566)
            ...
            self.assertEqual(values3[0].get("t"), self.timestamps_first[7])
            self.assertEqual(values3[0].get("v"), 9991234569)
            self.assertEqual(values3[1].get("t"), self.timestamps_first[9])
            self.assertEqual(values3[1].get("v"), 9991234563)

            # regionA, mobile
            self.assertEqual(records[4].get("productId"), "mobile")
            values4 = json.loads(records[4].get("phoneNumbers")).get("c")
            self.assertEqual(len(values4), 3)
            self.assertEqual(values4[0].get("t"), self.timestamps_first[0])
            self.assertEqual(values4[0].get("v"), 9991234567)
            self.assertEqual(values4[1].get("t"), self.timestamps_second[4])
            self.assertEqual(values4[1].get("v"), 9991234563)
            self.assertEqual(values4[2].get("t"), self.timestamps_second[6])
            self.assertEqual(values4[2].get("v"), 8881234566)
    ...
    def test_deleting_context(self):
        with bios.login(ep_url(), f"{admin_user}@{TENANT_NAME}", admin_pass) as session:
            ...
            assert (
                "Constraint violation: Context cannot be deleted because"
                " other streams depend on it" in exc_info.value.message
            )
