        ...
        implements ConnectorSplitManager
{
            ...
            Constraint constraint)
    {
        ...
        if ((tableHandle.getTableKind() == BiosTableKind.CONTEXT) ||
                (tableHandle.getTableKind() == BiosTableKind.CONTEXT_RAW) ||
                (tableHandle.getTableKind() == BiosTableKind.CONTEXT_SKETCHES)) {
        ...
        // -----------------------------------------------------------------------------------------
        // ** Next, create splits with the ranges that we want to use for actual queries against bios.

        // * Get split size and align it to window size.
        long windowSizeSeconds = biosClient.getEffectiveWindowSizeSeconds(tableHandle);
        long splitSize = (tableHandle.getTableKind() == BiosTableKind.SIGNAL_RAW) ?
        ...
        while (nextStart < end) {
            ...
            if (nextStart + splitSize <= currentTimeWithLag) {
                ...
                final var minimumAlignment =
                        (tableHandle.getTableKind() == BiosTableKind.SIGNAL_RAW) ?
