...
export DB_HEAPDUMP_DIR=/var/lib/db/heapdump
if [ "x$DB_HEAPDUMP_DIR" != "x" ]; then
    JVM_OPTS="$JVM_OPTS -XX:HeapDumpPath=$DB_HEAPDUMP_DIR/db-`date +%s`-pid$$.hprof"
fi
...
# Initializing these values to make sure JMX port is also secured irrespective of it is local or remote connection.
JVM_OPTS="$JVM_OPTS -Djavax.net.ssl.keyStore=/var/ext_resources/db.pks12.keystore"
JVM_OPTS="$JVM_OPTS -Djavax.net.ssl.keyStorePassword=secret"
JVM_OPTS="$JVM_OPTS -Djavax.net.ssl.trustStore=/var/ext_resources/db.truststore"
JVM_OPTS="$JVM_OPTS -Djavax.net.ssl.trustStorePassword=secret"

# this will make JMX authentication true irrespective of local/remote connection.
...
JVM_OPTS="$JVM_OPTS -Dcom.sun.management.jmxremote.password.file=$DB_HOME/conf/jmxremote.password"
...
JVM_OPTS="$JVM_OPTS -Djava.library.path=$DB_HOME/lib/sigar-bin"
