...
def window_start_time_to_millis(src):
    return int(datetime.strptime(src, "%Y-%m-%dT%H:%M:%S%z").timestamp() * 1000)
...
def process_alert(
    ...
    user = "observe_read_write@isima.io"
    password = "NotInUse"
    slack_url_low = (
        "https://hooks.slack.com/services/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"
    )
    slack_url_high = (
        "https://hooks.slack.com/services/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"
    )
    time_zone_for_start_time_1 = "US/Pacific" or "UTC"
    time_zone_for_start_time_2 = "Asia/Kolkata"
    time_zone_for_start_time_3 = ""
    reports = {
        "highCpuUsage1": [
            {"reportId": "15.CPU_Usage_by_Host", "durationMinutes": 60},
            {"reportId": "14.Container_CPU_Usage", "durationMinutes": 60},
        ],
        "highCpuUsage2": [
            {"reportId": "15.CPU_Usage_by_Host", "durationMinutes": 60},
            {"reportId": "14.Container_CPU_Usage", "durationMinutes": 60},
        ...
        "highMemoryUsage1": [
            {"reportId": "Container_Memory_Usage", "durationMinutes": 60},
            {"reportId": "13.Memory_Utilization", "durationMinutes": 60},
        ],
        "highMemoryUsage2": [
            {"reportId": "Container_Memory_Usage", "durationMinutes": 60},
            {"reportId": "13.Memory_Utilization", "durationMinutes": 60},
        ...
        "biosHeartbeat15Min1": [
            {"reportId": "Heartbeat_bios", "durationMinutes": 60},
            {"reportId": "Heartbeat_All", "durationMinutes": 180},
        ],
        "biosHeartbeat30Min2": [
            {"reportId": "Heartbeat_bios", "durationMinutes": 60},
            {"reportId": "Heartbeat_All", "durationMinutes": 180},
        ],
        "highDiskSpaceUsage1": [{"reportId": "Disk_Space_Usage", "durationMinutes": 360}],
        "highDiskSpaceUsage2": [{"reportId": "Disk_Space_Usage", "durationMinutes": 360}],
        "highDiskIoReadLatency1": [{"reportId": "Disk_Read_Latency", "durationMinutes": 60}],
        "highDiskIoWriteLatency1": [{"reportId": "Disk_Write_Latency", "durationMinutes": 60}],
        "highNetworkErrors1": [{"reportId": "Network_Errors", "durationMinutes": 60}],
        "highLbErrors1": [{"reportId": "lbRequest_Errors", "durationMinutes": 60}],
        "highLbErrors2": [{"reportId": "lbRequest_Errors", "durationMinutes": 60}],
        "biosdbIsDown1": [
            {"reportId": "DB_Is_Up", "durationMinutes": 60},
            {"reportId": "DB_Number_of_Files", "durationMinutes": 60},
        ],
        "biosdbIsDown2": [
            {"reportId": "DB_Is_Up", "durationMinutes": 60},
            {"reportId": "DB_Number_of_Files", "durationMinutes": 60},
    ...
    alert_periods = {
        "highCpuUsage1": {"silence_days": 1, "caution_days": 2},
        "highCpuUsage2": {"silence_days": 0.2, "caution_days": 0.5, "low_reminders": 2},
        "logErrorBios1": {"silence_days": 0.04},
        "logErrorBios2": {"silence_days": 0.25},
        "logErrorBiosdb1": {"silence_days": 0.04},
        "logErrorBiosdb2": {"silence_days": 0.25},
        "logErrorBioslb1": {"silence_days": 0.25},
        "logErrorBioslb2": {"silence_days": 0.25},
        "logErrorApps1": {"silence_days": 0.25},
        "logErrorAny1": {"silence_days": 0.25},
        "logWarningAny1": {"silence_days": 0.25},
    }
    ...
    elif alert_name_core == "biosdbIsDown":
    ...
    if alert_name in reports:
        ...
        for report in reports_for_alert:
            description += (
                prefix
                + f"<https://{cluster_dns_name}/report/{report['reportId']}"
                + f"/{report['durationMinutes'] * 60 * 1000}|{report['reportId']}>"
            )
            prefix = "\n                 "
    description += f"\n*Duration:* {window_length}\n" + start_time_string
    description += (
        f"\n*Signal:* <https://{cluster_dns_name}/signal/{signal_name}|{signal_name}>"
        + f"\n*Condition:* {condition}\n"
    ...
    message["attachments"] = []
    message["attachments"].append({"color": "#D0E0EC"})
    blocks = [
        {
            "type": "section",
            "text": {
...
def add_logs(event, session, blocks, current_time):
    max_logs_to_show = int("5")
    max_log_digests_to_show = int("5")
    logs_period_seconds = int("330")
    log_digests_period_minutes = int("10")
