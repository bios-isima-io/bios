...
public class IpBlacklistStream implements IStreamProvider {
  private static final Logger logger = LoggerFactory.getLogger(IpBlacklistStream.class);

  private static final String DEFAULT_SCHEMA_JSON_FILE = "ipblacklist_context.json";

  private StreamConfig ipBlacklistContextStream;

  public IpBlacklistStream() {
    this(DEFAULT_SCHEMA_JSON_FILE);
  }

  IpBlacklistStream(String schemaJsonFile) {
    StreamConfig temp = getStreamConfigFromFile(schemaJsonFile);
    if (temp == null) {
      final String msg = "Failed to load ip2geo stream config, file :" + schemaJsonFile;
      logger.error(msg);
    }
    ipBlacklistContextStream = temp;
  }
  ...
  public StreamConfig getStreamConfig() {
    return ipBlacklistContextStream;
  }
}
