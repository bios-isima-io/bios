...
/**
 * This class denotes ClosingParenthesisState in tokenizer DFA. It has below behavior 1) Transition
 * to StartState on any valid token 2) Throw InvalidRuleException on invalid token
 */
public class ClosingParenthesisState implements TokenizerState {
  private final List<Token> currentTokenList;
  private final StringBuilder currentTokenString;

  public ClosingParenthesisState(List<Token> tokenList, StringBuilder currentTokenString) {
  ...
  public TokenizerState consumeChar(char nextChar) throws InvalidRuleException {
    ...
    if (tokenType.equals(TokenType.INVALID)) {
      ...
      currentTokenList.add(new Token(TokenType.PARENTHESIS_CLOSE, currentTokenString.toString()));
