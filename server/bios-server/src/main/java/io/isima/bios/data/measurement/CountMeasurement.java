...
public class CountMeasurement extends Measurement {
  ...
  public AttributeValue get() {
    return count > 0 ? new AttributeValueGeneric(count, AttributeType.INTEGER) : null;
  }

  @Override
  public void merge(Measurement merging) {
    if (merging instanceof CountMeasurement) {
      count += ((CountMeasurement) merging).count;
    } else {
